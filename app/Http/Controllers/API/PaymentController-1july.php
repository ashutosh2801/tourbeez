...
class PaymentController extends Controller
{
    ...
    public function verifyPayment(Request $request)
    {
        ...
        $request->validate([
            ...
            'action_name'   => 'required|in:book,reserve',
        ]);
        ...
        try {
            ...
            $actionName   = $request->action_name ?? 'book';
            ...
            if ($actionName === 'book') {
                ...
                if ($booking->order_status === 6) {
                    return response()->json([
                        'status'  => 'cancelled',
                        'message' => 'Your order was already cancelled! Please check your email for order details.',
                        'booking' => [],
                    ]);
                }
                ...
                if ($paymentIntent->status === 'requires_capture') {
                    ...
                    $bookedAmount      = $booking->booked_amount;
                    $flagPaymentStatus = 1;
                ...
                $paymentMethodName = $paymentIntent->payment_method_types[0] ?? 'card';

                if ($paymentIntent->last_payment_error?->payment_method?->type) {
                    $paymentMethodName = $paymentIntent->last_payment_error->payment_method->type;
                }
                ...
                if (!empty($paymentIntent->payment_method)) {
                    try {
                        ...
                        OrderPayment::updateOrCreate(
                            ...
                            [
                                ...
                                'card_exp_year'     => $paymentMethod->card->exp_year ?? null,
	                                ...
	                                'currency'          => $paymentIntent->currency ?? $booking->currency,
	                                'current_rate'      => $booking->current_rate ?? 1,
                        ...
                        Log::warning('Payment method details not saved for PI ' . $paymentIntent->id . ': ' . $cardError->getMessage());
                    }
                }

                $responseStatus = $paymentIntent->status;
                ...
                ->where('payment_intent_id', $setupIntent->id)
                ->first();
                ...
                if ($setupIntent->status !== 'succeeded') {
                    return response()->json([
                        'status'  => 'failed',
                        'message' => 'Card setup was not completed. Please try again.',
                        'booking' => [],
                    ]);
                }
                        ...
                        OrderPayment::updateOrCreate(
                            ...
                            [
                                ...
                                'card_exp_year'     => $paymentMethod->card->exp_year ?? null,
	                                ...
	                                'status'            => 'reserve',
                                'action'            => $actionName,
                                'response_payload'  => json_encode($setupIntent),
                                'collection_date'   => now(),
                            ]
                        );
                    } catch (\Exception $cardError) {
                        Log::warning('Payment method details not saved for SI ' . $setupIntent->id . ': ' . $cardError->getMessage());
                    }
                }
            ...
            if ($booking->order_status === 5 || $booking->order_status === 6) {
                $orderStatus = $booking->order_status;
            }

            $booking->total_amount   = $totalAmount;
            $booking->balance_amount = $balanceAmount;
            $booking->booked_amount  = $bookedAmount;
            ...
            $pricing = [];
            $tourPricing = json_decode($booking->order_tour?->tour_pricing ?? '[]');

            if (!empty($tourPricing) && is_array($tourPricing)) {
                foreach ($tourPricing as $tp) {
                    $tourPrice = TourPricing::find($tp->tour_pricing_id ?? null);

                    $label = $tourPrice?->label ?? '';
                    $label = str_ireplace('Group', 'Participants', $label);

                    $pricing[] = [
                        'label'        => $label,
                        'price_type'   => $tp->price_type ?? null,
                        'qty'          => $tp->quantity ?? 0,
                        'price'        => $tp->price ?? 0,
                        'actual_price' => $tp->actual_price ?? ($tp->price ?? 0),
                        'discount'     => $tp->discount ?? 0,
                        'total'        => $tp->total_price ?? 0,
                    ];
                }
            }

            self::syncPromoPaymentForBooking($booking, $tourPricing);

            $extra = [];
            $extraPricing = json_decode($booking->order_tour?->tour_extra ?? '[]');

            if (!empty($extraPricing) && is_array($extraPricing)) {
                foreach ($extraPricing as $ep) {
                    $extraAddon = Addon::find($ep->tour_extra_id ?? null);

                    $extra[] = [
                        'label' => $extraAddon?->name ?? '',
                        'qty'   => $ep->quantity ?? 0,
                        'price' => $ep->price ?? 0,
                        'total' => $ep->total_price ?? 0,
                    ];
                }
            }

            $fees = [];
            $feesPricing = json_decode($booking->order_tour?->tour_fees ?? '[]');

            if (!empty($feesPricing) && is_array($feesPricing)) {
                foreach ($feesPricing as $fp) {
                    $labelText = ($fp->label ?? '');

                    if (isset($fp->type, $fp->value)) {
                        $labelText .= $fp->type == 'PERCENT'
                            ? ' (' . $fp->value . '%)'
                            : ' (' . $fp->value . ')';
                    }
            ...
            $detail = [
                ...
                'pickup'           => $pickName,
                'tour_date'        => $booking->order_tour?->tour_date
                                        ? date('D, M d, Y', strtotime($booking->order_tour->tour_date))
                                        : '',
            ...
            if ($booking && !$booking->tour?->order_email && !$booking->email_sent) {
                self::sendOrderDetailMail($detail, $actionName);

                OrderActions::insert([
                    'order_id'     => $booking->id,
                    'performed_by' => $booking->customer?->id,
                    'notes'        => ($booking->customer?->name ?? 'Customer') . " Pending order mail sent {$booking->order_number}",
                    'created_at'   => now(),
                    'updated_at'   => now(),
                ]);
            ...
            $admin = $shouldNotifyAdmin ? User::find($booking->tour?->user_id) : null;
    ...
    public function verifyPayment30june(Request $request)
    {
        Log::info('verifyPayment');
    ...
    private static function syncPromoPaymentForBooking($booking, array $tourPricing): void
    {
        $promoCode = trim((string) ($booking->customer?->promo_code ?? ''));

        if ($promoCode === '') {
            return;
        }

        $existingPromoAmount = $booking->payments()
            ->where('status', 'succeeded')
            ->where('payment_type', 'PROMO_CODE')
            ->sum('amount');

        if ($existingPromoAmount > 0) {
            return;
        }
        ...
        foreach ($tourPricing as $item) {
            ...
            if (
                !str_contains($label, 'adult') &&
                !str_contains($label, 'participant') &&
                !str_contains($label, 'group')
            ) {
                continue;
            }

            $pricingTotal += $lineTotal;

            switch ($promo->value_type) {
                case 'VALUE_LIMITPRODUCT':
                    if (($item->tour_pricing_id ?? null) == $promo->product_id) {
                        $discountAmount = min((float) $promo->voucher_value, $lineTotal);
                    }
                    break;

                case 'VALUE_LIMITCATEGORY':
                    $product = TourPricing::find($item->tour_pricing_id ?? null);
                ...
                case 'PERCENT_LIMITPRODUCT':
                    if (($item->tour_pricing_id ?? null) == $promo->product_id) {
                        $discountAmount = ($lineTotal * (float) $promo->value_percent) / 100;
                    }
                    break;
        ...
        if ($promo->value_type === 'VALUE') {
            $discountAmount = min((float) $promo->voucher_value, $pricingTotal);
        } elseif ($promo->value_type === 'PERCENT') {
            $discountAmount = ($pricingTotal * (float) $promo->value_percent) / 100;
        }
        ...
        $availableCreditRoom = max((float) $booking->total_amount - (float) $paidAmount, 0);
        $discountAmount = round(min($discountAmount, $pricingTotal, $availableCreditRoom), 2);
        ...
        OrderPayment::updateOrCreate(
            ...
            [
                ...
                'response_payload'  => json_encode([
                    ...
                    'source'        => 'verify_payment_reconcile',
                ]),
            ]
        );

        $booking->balance_amount = max((float) $booking->total_amount - (float) $paidAmount - $discountAmount, 0);
        $booking->save();
    }
