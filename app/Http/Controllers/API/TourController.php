...
private function getNextAvailableDate(int $tourId): ?array
{
    $today = Carbon::today();
    ...
    $nextDates = $schedules
        ->map(fn($schedule) => $this->calculateNextDate($schedule, $today))
        ->filter()
        ->values(); // reset keys
    ...
    // Return the array with the earliest date
    return $nextDates->sortBy('date')->first();
}
...
//     $next = $today->greaterThan($scheduleStartDate) 
//         ? $today->copy() 
//         : $scheduleStartDate->copy();
...
//             $allowed = $schedule->repeats()->where('tour_schedule_id', $schedule->id)
//                 ->where('day', $dayName)
//                 ->exists();
...
private function calculateNextDate($schedule, Carbon $today): ?array
{
    $interval   = $schedule->repeat_period_unit ?? 1;
    $repeatType = $schedule->repeat_period;
    ...
    // ✅ preload repeats into memory (no queries in loop)
    $allowedDays = $schedule->repeats->pluck('day')->toArray();

    while ($next->lte($scheduleEndDate)) {

        // WEEKLY / HOURLY / MINUTELY checks
        if (in_array($repeatType, ['WEEKLY', 'MINUTELY', 'HOURLY'])) {
            $dayName = $next->format('l');
            ...
            if ($repeatType === 'WEEKLY') {
                $weekDiff = $scheduleStartDate->diffInWeeks($next);
                if ($weekDiff % $interval !== 0) {
                    // ✅ skip whole week instead of day-by-day
                    $next->addWeek();
                    continue;
                }
            }
        }
        $repeatsForDay = $schedule->repeats->keyBy('day');
        if ($this->hasValidSlot($schedule, $next,30,0, $repeatsForDay)) {
            return ['date' => $next->toDateString()];
        }
        ...
        switch ($repeatType) {
            case 'DAILY':    $next->addDays($interval); break;
            case 'WEEKLY':   $next->addDay(); break; // already optimized skipping above
            case 'MONTHLY':  $next->addMonths($interval); break;
            case 'YEARLY':   $next->addYears($interval); break;
            case 'HOURLY':   $next->addHours($interval); break;
            case 'MINUTELY': $next->addMinutes($interval); break;
...
// private function hasValidSlot($schedule, Carbon $date, $durationMinutes = 30, $minimumNoticePeriod = 0, $repeatsForDay = null)
// {
...
// }
    ...
    private function hasValidSlot($schedule, Carbon $date, $durationMinutes = 30, $minimumNoticePeriod = 0, $repeatsForDay = null // pass preloaded repeats if available
    ) {
        $repeatType = $schedule->repeat_period;

        // Default session start/end
        $startTime = Carbon::parse($date->format('Y-m-d') . ' ' . $schedule->session_start_time);
        $endTime   = Carbon::parse($date->format('Y-m-d') . ' ' . $schedule->session_end_time);

        if (in_array($repeatType, ['MINUTELY', 'HOURLY']) && $repeatsForDay) {
            $dayName = $date->format('l');
            if (isset($repeatsForDay[$dayName])) {
                $repeat = $repeatsForDay[$dayName];
                $startTime = Carbon::parse($date->format('Y-m-d') . ' ' . $repeat->start_time);
                $endTime   = Carbon::parse($date->format('Y-m-d') . ' ' . $repeat->end_time);
            }
        }

        // Calculate earliest allowed considering notice period
        $minimumNoticePeriod = $schedule->minimum_notice_unit === "HOURS" 
            ? $schedule->minimum_notice_num * 60 
            : $schedule->minimum_notice_num;

        $earliestAllowed = now()->addMinutes($minimumNoticePeriod);

        // Entire session in the past
        if ($endTime->lt($earliestAllowed)) {
            return false;
        }

        // Daily/weekly/monthly/yearly → simple check
        if (in_array($repeatType, ['DAILY', 'WEEKLY', 'MONTHLY', 'YEARLY'])) {
            return $startTime->gte($earliestAllowed);
        }

        // Hourly/minutely → iterate slots
        while ($startTime->lte($endTime)) {
            if ($startTime->gte($earliestAllowed)) {
                return true;
            }
     ...
     */
    private function isDateAvailable2343($schedule, $date, array $repeatsByDay = []): bool
    {
        // Hard bounds
...
// Round $m0 up to the nearest multiple of repeatUnit
    ...
    private function isDateAvailable($schedule, $date, array $repeatsByDay = []): bool
    {
        ...
        // ---- run original availability logic ----
        $available = false;
        ...
        if ($repeatType === 'NONE') {
            ...
            $monthsSinceStart = (int) floor($startDate->diffInMonths($date));
            if ($monthsSinceStart % $repeatUnit === 0 && (int)$date->format('d') === (int)$startDate->format('d')) {
                $slotStart = $at($schedule->session_start_time ?? '00:00');
                $slotEnd   = $at($schedule->session_end_time   ?? '23:59');
                $available = $windowOk($slotStart, $slotEnd);
            }
            ...
            if ($yearsSinceStart % $repeatUnit === 0
                && (int)$date->format('d') === (int)$startDate->format('d')
                && (int)$date->format('m') === (int)$startDate->format('m')) {
                $slotStart = $at($schedule->session_start_time ?? '00:00');
                $slotEnd   = $slotStart->copy()->addMinutes(max(1, $durationMin));
                $available = $windowOk($slotStart, $slotEnd);
        ...
        } elseif ($repeatType === 'HOURLY' || $repeatType === 'MINUTELY') {
            $dayName = $date->format('l');
            $entries = $repeatsByDay[$dayName] ?? $repeatsByDay[$dayName] = TourScheduleRepeats::where('tour_schedule_id', $schedule->id)
                ->where('day', $dayName)
                ->get();

            $slots = []; // collect all slots here
            ...
            foreach ($entries as $rep) {
                ...
                // minutes difference from earliest allow
                $m0 = max(0, (int)ceil($slotStart0->diffInMinutes($earliestAllow, false)));

                // adjust to nearest multiple of repeatUnit
                $m = ($m0 % $repeatUnit === 0) ? $m0 : $m0 + ($repeatUnit - ($m0 % $repeatUnit));

                // first valid candidate slot
                $candidate = $slotStart0->copy()->addMinutes($m);

                // generate all slots up to slotEnd
            ...
            // if you want to keep flag for availability
            $available = count($slots) > 0;
        }

        // var_dump($available);
        // ---- now apply deletion filtering ----
        if ($available) {
            if(!empty($slots)){
                    $response = [
                    'data' => [
                        $dayStr => $slots // dummy slot to respect deletion rules
                    ]
                ];
            } else{
                $slots = [];
                ...
                while ($slotStart->lte($slotEnd)) {
                    ...
                    if ($slotStart->gte($earliestAllow)) {
                        $slots[] = $slotStart->format('g:i A');
                    }

                    $slotStart = $slotStart->copy()->addMinutes(60);
                }

                
                $storeDeleteSlot = $this->fetchDeletedSlot($schedule->tour_id);
            ...
            // dd($slotStart, $slotEnd, $earliestAllow);
    ...
    private function isDateAvailable23($schedule, $date, array $repeatsByDay = []): bool
    {
        // Hard bounds
        $startDate = Carbon::parse($schedule->session_start_date)->startOfDay();
        ...
        if ($repeatType === 'WEEKLY') {
            ...
            if ($weeksSinceStart % $repeatUnit === 0) {
                ...
                if (in_array($dayName, $repeatsByDay)) {
                    $slotStart = $at($schedule->session_start_time ?? '00:00');
                    if ($slotStart->gte($earliestAllow)) {
                        $slots[] = $slotStart->format('g:i A');
                    }
        ...
        if ($repeatType === 'MONTHLY') {
            ...
            if ($monthsSinceStart % $repeatUnit === 0 && $startDate->day === $date->day) {
                $slotStart = $at($schedule->session_start_time ?? '00:00');
                if ($slotStart->gte($earliestAllow)) {
                    $slots[] = $slotStart->format('g:i A');
                }
        ...
        if ($repeatType === 'HOURLY') {
            $slotStart = $at($schedule->session_start_time ?? '00:00');
            while ($slotStart->lte($date->copy()->endOfDay())) {
                if ($slotStart->gte($earliestAllow)) {
                    $slots[] = $slotStart->format('g:i A');
                }
                $slotStart->addHours($repeatUnit);
            }
        }
        ...
        if ($repeatType === 'MINUTELY') {
            ...
            while ($slotStart->lte($date->copy()->endOfDay())) {
                ...
                $slotStart->addMinutes($repeatUnit);
            }
        }

        // -------------------------------
        // ✅ Apply deleted slot filtering
        // -------------------------------
        $storeDeleteSlot = $this->fetchDeletedSlot($schedule->tour_id);
        ...
        $slots    = $response['data'][$dayStr] ?? [];

        // -------------------------------
        // ✅ Return true if any slot remains
        // -------------------------------
    ...
    private function getDisabledTourDates(int $tourId): array
    {
        ...
        if ($deleteTypes->contains('all')) {
            ...
            return [
                'disabled_tour_dates' => [], // ✅ correct global disabled
                'per_schedule' => [],          // ✅ meta for debugging
        ...
        // } elseif ($deleteTypes->contains('after')) {
        //     // $globalEnd = Carbon::today();
        // } 
